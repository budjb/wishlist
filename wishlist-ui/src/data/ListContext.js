...
export const ListProvider = ({ wishlistId, children }) => {
   ...
   */
  const deleteList = id => {
    return auth0Context.getTokenSilently().then(token => {
      return fetch(
        `${config.apiBaseUrl}/wishlists/${id}`, {
          method: 'DELETE',
          headers: {
  ...
  const addItem = data => {
    return auth0Context.getTokenSilently()
      .then(token => {
        return fetch(
          `${config.apiBaseUrl}/wishlists/${wishlistId}/items`, {
            method: 'POST',
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json'
  ...
  const updateItem = (id, data) => {
    return auth0Context.getTokenSilently()
      .then(token => {
        return fetch(
          `${config.apiBaseUrl}/wishlists/${wishlistId}/items/${id}`,
          {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    if (!auth0Context.loading) {
      getList().then(getItems).catch(err => {
        console.error(err);
        setUnavailable(true);
